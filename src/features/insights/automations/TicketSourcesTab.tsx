// Ticket source tab body: a ticket header (id link, channel chip, status,
...
export function TicketSourcesTab({ detail }: { detail: GeneratedUseCaseDetail }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-3">
        <p className="text-[12px] font-medium text-grey-700">
          Date created: <span className="text-black">{ticket.dateCreated}</span>
        </p>
        <div className="flex items-center gap-2">
          <span className="text-[12px] font-medium text-grey-700">Channel:</span>
          <span
      ...
      <div className="grid grid-cols-3 overflow-hidden rounded-[20px] border border-grey-200">
        {ticket.metrics.map((m, metricIndex) => (
          <div
            key={m.label}
            className={
              'p-4 ' +
              (metricIndex % 3 === 2 ? '' : 'border-r border-grey-200 ') +
              (metricIndex < 3 ? 'border-b border-grey-200' : '')
            }
          >
            <p className="text-[12px] text-grey-800">{m.label}</p>
      ...
      <section className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-4 rounded-[20px] bg-[#f9f8f7] p-4">
          <Card flat className="flex items-center justify-between rounded-xl bg-white/70 px-4 py-2">
            <span className="flex items-center gap-2 text-[12px] font-semibold text-black">
              <ArrowLeft size={16} aria-hidden />
              Customer request
            </span>
            <span className="text-[12px] text-grey-700">{ticket.customerRequest.timestamp}</span>
          </Card>
          <div className="flex flex-col gap-1">
            <p className="text-[12px] font-medium text-grey-700">Subject:</p>
          ...
          <Card flat className="flex items-center justify-between rounded-xl bg-white/70 px-4 py-2">
            ...
            <span className="text-[12px] text-grey-700">{ticket.agentResponse.timestamp}</span>
          </Card>
          <p className="whitespace-pre-wrap text-[14px] leading-5 text-black">
