...
describe('TopicsView', () => {
  it('renders the Figma Topics dashboard zones', () => {
    render(<TopicsView />)
    ...
    expect(view.getAllByText('Last 30 days')).toHaveLength(2)
    expect(view.getByText('Total tickets')).toBeInTheDocument()
  ...
  it('collapses the top-movers panel', async () => {
    ...
    render(<TopicsView />)
    const view = within(screen.getByTestId('view-cx-topics'))
    expect(view.getByText('Top movers by categorized tickets')).toBeInTheDocument()
    await user.click(view.getByRole('button', { name: 'Toggle top movers' }))
    expect(view.queryByText('Top movers by categorized tickets')).not.toBeInTheDocument()
  })

  it('collapses the overview metrics while keeping the topics table visible', async () => {
    const user = userEvent.setup()
    render(<TopicsView />)
    const view = within(screen.getByTestId('view-cx-topics'))
    expect(view.getByText('Total tickets')).toBeInTheDocument()
    await user.click(view.getByRole('button', { name: 'Collapse overview' }))
    expect(view.queryByText('Total tickets')).not.toBeInTheDocument()
    expect(view.getByText('Account Management')).toBeInTheDocument()
    await user.click(view.getByRole('button', { name: 'Expand overview' }))
    expect(view.getByText('Total tickets')).toBeInTheDocument()
  })
