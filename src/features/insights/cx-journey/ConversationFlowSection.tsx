...
import { SankeySheen, sankeyMotionStyle } from '@/components/sankey/SankeyMotion'
...
function buildLayout(width: number, height: number) {
  ...
  const columns = [0, 1, 2, 3].map((c) =>
    nodes.map((n, i) => ({ n, i })).filter((o) => o.n.col === c),
  )
...
function FlowSankey({ width, height }: { width: number; height: number }) {
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="Conversation handling flow"
    >
      {ribbons.map((r, i) => (
        <path
          key={i}
      ...
      {FLOW_SANKEY.nodes.map((node, i) => {
        ...
        // Keep the compact name/value stack centered on its corresponding bar.
        return (
          <g key={node.name}>
            <rect
              x={p.x}
            ...
            />
            <g className="sankey-label" data-sankey-label style={sankeyMotionStyle(node.col)}>
              <text
                x={labelX}
                y={labelY - 3}
                textAnchor={anchor}
      ...
      <div className="rounded-2xl bg-app-backdrop p-6">
        ...
        <div ref={ref} className="h-[260px] w-full">
          {size.width > 0 && size.height > 0 && (
            <FlowSankey width={size.width} height={size.height} />
          )}
        </div>
      </div>
    </section>
