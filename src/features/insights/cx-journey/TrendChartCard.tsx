...
export function TrendChartCard({ chart, granularity }: { chart: TrendChart; granularity: Granularity }) {
  ...
  // Bars animate by default in recharts, but on its own 1500ms 'ease' — twice as
  // long as anything else in the app, and blind to a reduced-motion preference.
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 4, right: 4, bottom: 0, left: 4 }} barSize={18}>
            ...
            <Bar
              ...
              stackId="a"
              fill={BLUE}
              radius={chart.stacked ? [0, 0, 0, 0] : [3, 3, 0, 0]}
              {...motion}
            />
            {chart.stacked ? (
              <Bar dataKey="value2" stackId="a" fill={RED} radius={[3, 3, 0, 0]} {...motion} />
            ) : null}
          </BarChart>
        </ResponsiveContainer>
      </div>
