...
describe('conversations-data', () => {
  ...
  it('headless carries its six generic card titles', () => {
    const titles = CHANNELS.headless.cards.map((c) => c.title)
  ...
  it('non-headless channels share the generic template and drop Deflections', () => {
    const shared = [
      'Total conversations',
    ...
    for (const k of ['widget', 'voice', 'webcall'] as ChannelKey[]) {
      const titles = CHANNELS[k].cards.map((c) => c.title)
      for (const s of shared) expect(titles).toContain(s)
      expect(titles).not.toContain('Deflections')
    }
  })

  it('non-headless channels expose User query / Resolved columns instead of Detected agents', () => {
    for (const k of ['widget', 'voice', 'webcall'] as ChannelKey[]) {
      const cols = CHANNELS[k].columns.map((c) => c.id)
      expect(cols).toContain('query')
      expect(cols).toContain('resolved')
      expect(cols).not.toContain('agents')
    }
  })
  ...
  it('every row carries a populated detail', () => {
    for (const k of KEYS) {
      for (const row of CHANNELS[k].rows) {
        ...
        expect(row.detail.criteria.length).toBeGreaterThan(0)
      }
    }
  })
