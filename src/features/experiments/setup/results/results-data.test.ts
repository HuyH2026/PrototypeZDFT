...
describe('experiment details', () => {
  ...
  it('uses the updated resolution metric labels in the default result', () => {
    expect(DEFAULT_EXPERIMENT_DETAIL.metricCards[0]?.title).toBe('Resolutions')
    expect(DEFAULT_EXPERIMENT_DETAIL.metricCards[1]?.title).toBe('Resolution rate')
  })
})
