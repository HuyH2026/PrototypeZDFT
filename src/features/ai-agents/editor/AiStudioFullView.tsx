        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col gap-0.5">
            <span className="flex items-center gap-1.5 text-[14px] font-semibold leading-5 text-black">
              {plan.title}
              <span className="size-1 rounded-full bg-grey-800" aria-hidden />
              <span className="text-grey-800">{plan.channel}</span>
            </span>
            <span className="text-[12px] leading-[18px] text-grey-800">{plan.planSubtitle}</span>
          </div>
        </div>
        {approved && (
...
export function AiStudioFullView({
  onClose, onApprove, initialComposer = '',
}: {
  onClose: () => void
      ...
      >
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          ...
          <div className="flex shrink-0 items-center gap-4 px-6 py-5">
            <Menu size={24} className="text-grey-800" aria-hidden />
            <div className="flex items-center gap-0.5">
              <ZendeskLogo size={24} />
              <span className="ml-1 text-[18px] font-semibold text-grey-800">AI Studio</span>
          ...
          <div className="flex-1 overflow-y-auto px-20 pb-4">
            ...
            <div className="mb-2 flex items-center gap-2 py-2">
              <span className="text-[14px] font-medium leading-5 text-grey-700">
                {AI_STUDIO_ANALYSIS.thinkingLabel}
              </span>
              <ChevronRight size={18} className="text-grey-700" aria-hidden />
            </div>
            ...
            {approved && (
              <div ref={workingRef} className="mt-6 flex items-center gap-2 scroll-mb-4">
                <ZendeskLogo size={24} className="text-grey-800" />
                <span className="animate-pulse text-[14px] font-medium text-grey-800">
                  {plan.workingLabel}
                </span>
              </div>
            )}
            ...
            <div className="flex items-center gap-2 rounded-full border border-[#ffb393] bg-white px-4 py-3">
              ...
              >
                <svg width={24} height={24} viewBox="0 0 24 24" fill="none" aria-hidden>
                  <path d="M4 12l16-8-6 16-3-6-7-2z" stroke="#646864" strokeWidth={1.5} strokeLinejoin="round" strokeLinecap="round" />
                </svg>
              </button>
            </div>
        ...
        <div className="m-3 flex w-[590px] shrink-0 flex-col overflow-hidden rounded-[20px] bg-white/65">
          <div className="flex shrink-0 items-center justify-between px-6 py-5">
            <div className="flex items-center gap-2">
              ...
              <span className="flex items-center gap-1 text-[12px] font-medium text-grey-800">
                {plan.title}
                <span className="size-[3px] rounded-full bg-grey-800" aria-hidden />
                {plan.channel}
              </span>
            </div>
            <button
          ...
          <div className="flex-1 overflow-y-auto px-6 pb-8">
            ...
            {/* Summary card */}
            <div
              className="mb-8 flex flex-col gap-2.5 rounded-xl border border-grey-100 p-4"
            ...
            <div className="mb-8 flex gap-3">
              <span className="w-9 shrink-0 text-[18px] font-medium leading-[22px] text-grey-800">{s1.number}</span>
              <div className="flex flex-1 flex-col gap-2">
                <h3 className="text-[18px] font-semibold leading-[22px] text-black">{s1.title}</h3>
                <p className="text-[14px] leading-5 text-grey-800">{s1.body}</p>
              </div>
            </div>
            ...
            {plan.sections.slice(1).map((section) => (
              <div key={section.number} className="mb-6 flex gap-3 border-t border-surface-border pt-6">
                <span className="w-9 shrink-0 text-[18px] font-medium leading-[22px] text-grey-800">{section.number}</span>
                <div className="flex flex-1 flex-col gap-2">
                  <h3 className="text-[18px] font-semibold leading-[22px] text-black">{section.title}</h3>
                  <p className="text-[14px] leading-5 text-grey-800">{section.body}</p>
                </div>
              </div>
            ))}
          </div>
