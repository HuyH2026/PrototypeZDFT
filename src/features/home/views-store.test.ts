...
import { DEFAULT_EXECUTIVE_LAYOUT } from './executive-data'
...
describe('views-store', () => {
  ...
  describe('load from localStorage', () => {
    ...
    it('REGRESSION: a full page reload clears persisted views back to a fresh single Default', async () => {
      ...
      const postLoad = addFresh(loaded, { name: 'Post-reload', role: 'cs', layout: { left: ['cost'], right: [] } })
...
const genPm: NewView = { name: 'Product lifecycle', kind: 'pm', role: 'pm', pmLayout: [...DEFAULT_PM_LAYOUT] }
...
describe('views-store — Executive views', () => {
  it('creates an executive view without replacing Default', () => {
    const s = addView(seedViewsState(), genExecutive)
    expect(s.views).toHaveLength(2)
    expect(s.views[0].name).toBe('Default')
    const view = getActiveView(s)
    expect(view.kind).toBe('executive')
    if (view.kind === 'executive') {
      expect(view.executiveLayout).toEqual(DEFAULT_EXECUTIVE_LAYOUT)
      expect(view.role).toBe('exec')
    }
  })

  it('reorders, removes, and restores executive sections', () => {
    let s = addView(seedViewsState(), genExecutive)
    s = moveExecutiveSection(s, 0, 2)
    let view = getActiveView(s)
    if (view.kind !== 'executive') throw new Error('expected executive view')
    expect(view.executiveLayout[0]).toBe('customer-outcomes')

    s = removeExecutiveSection(s, 'business-value')
    view = getActiveView(s)
    if (view.kind !== 'executive') throw new Error('expected executive view')
    expect(view.executiveLayout).not.toContain('business-value')

    s = addExecutiveSection(s, 'business-value')
    view = getActiveView(s)
    if (view.kind !== 'executive') throw new Error('expected executive view')
    expect(view.executiveLayout).toContain('business-value')
  })

  it('reset restores the complete executive layout', () => {
    let s = addView(seedViewsState(), genExecutive)
    s = removeExecutiveSection(s, 'leadership-action')
    s = resetExecutiveLayout(s)
    const view = getActiveView(s)
    if (view.kind === 'executive') {
      expect(view.executiveLayout).toEqual(DEFAULT_EXECUTIVE_LAYOUT)
    }
  ...
  it('executive reducers are no-ops for the Default grid view', () => {
    const s = seedViewsState()
    expect(moveExecutiveSection(s, 0, 1)).toBe(s)
    expect(removeExecutiveSection(s, 'outcomes')).toBe(s)
    expect(addExecutiveSection(s, 'outcomes')).toBe(s)
    expect(resetExecutiveLayout(s)).toBe(s)
  })
})
