...
// pattern as src/app/brand-context.tsx.
...
export type ExecutiveView = {
  id: string; name: string; kind: 'executive'
  role: 'exec'; executiveLayout: ExecutiveSectionId[]
  builtIn?: boolean
}
export type DashboardView = GridView | PmView | ExecutiveView
...
export type NewExecutiveView = {
  name: string; kind: 'executive'; role: 'exec'; executiveLayout: ExecutiveSectionId[]
}
export type NewView = NewGridView | NewPmView | NewExecutiveView
...
const EXECUTIVE_SECTION_IDS = new Set<string>(EXECUTIVE_SECTION_ID_LIST)
const ROLE_KEYS = new Set<string>(['ops', 'pm', 'cs', 'knowledge', 'exec'])
...
function sanitizeExecutiveLayout(arr: unknown): ExecutiveSectionId[] {
  if (!Array.isArray(arr)) return [...DEFAULT_EXECUTIVE_LAYOUT]
  const seen = new Set<ExecutiveSectionId>()
  const cleaned = arr.filter((x): x is ExecutiveSectionId =>
    typeof x === 'string' &&
    EXECUTIVE_SECTION_IDS.has(x) &&
    !seen.has(x as ExecutiveSectionId) &&
    seen.add(x as ExecutiveSectionId) !== undefined,
  )
  return cleaned.length > 0 ? cleaned : [...DEFAULT_EXECUTIVE_LAYOUT]
}
...
function sanitizeView(v: unknown): DashboardView | null {
  ...
  if (o.kind === 'executive') {
    return {
      ...
      kind: 'executive',
      role: 'exec',
      executiveLayout: sanitizeExecutiveLayout(o.executiveLayout),
      ...builtIn,
    }
  }
  if (o.kind === 'pm') {
...
export function addView(state: ViewsState, view: NewView): ViewsState {
  ...
  const created: DashboardView =
    view.kind === 'executive'
      ? { id, name: view.name, kind: 'executive', role: 'exec', executiveLayout: [...view.executiveLayout] }
      : view.kind === 'pm'
...
// --- Executive view reducers ------------------------------------------------
function mapActiveExecutive(
  state: ViewsState,
  fn: (view: ExecutiveView) => ExecutiveView,
): ViewsState {
  const active = getActiveView(state)
  if (active.kind !== 'executive') return state
  return {
    ...state,
    views: state.views.map((view) =>
      view.id === state.activeId && view.kind === 'executive' ? fn(view) : view,
    ),
  }
}

export function moveExecutiveSection(
  state: ViewsState,
  fromIndex: number,
  toIndex: number,
): ViewsState {
  return mapActiveExecutive(state, (view) => {
    const next = [...view.executiveLayout]
    if (fromIndex < 0 || fromIndex >= next.length) return view
    const [moved] = next.splice(fromIndex, 1)
    const clamped = Math.max(0, Math.min(toIndex, next.length))
    next.splice(clamped, 0, moved)
    return { ...view, executiveLayout: next }
  })
}

export function removeExecutiveSection(
  state: ViewsState,
  id: ExecutiveSectionId,
): ViewsState {
  return mapActiveExecutive(state, (view) => ({
    ...view,
    executiveLayout: view.executiveLayout.filter((section) => section !== id),
  }))
}

export function addExecutiveSection(
  state: ViewsState,
  id: ExecutiveSectionId,
): ViewsState {
  return mapActiveExecutive(state, (view) =>
    view.executiveLayout.includes(id)
      ? view
      : { ...view, executiveLayout: [...view.executiveLayout, id] },
  )
}

export function resetExecutiveLayout(state: ViewsState): ViewsState {
  return mapActiveExecutive(state, (view) => ({
    ...view,
