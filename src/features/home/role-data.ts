...
// reordered and the health narrative reframed for that role. role === null (the
...
const ROLE_SUMMARY: Record<GridRole, string> = {
  ...
  cs: 'Satisfaction is climbing and escalations are down 5.8%, so fewer conversations are reaching your team. Voice handle time is the outlier.',
...
export function deriveRoleData(base: LevelData, role: Role | null): LevelData {
  ...
  return {
    ...
    healthDigest: {
      ...base.healthDigest,
      narrative: ROLE_SUMMARY[role] ?? base.healthDigest.narrative,
    },
  }
}
