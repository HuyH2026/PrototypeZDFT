...
describe('pm-data', () => {
  ...
  describe('pm-data — spotlight → opportunity links', () => {
    ...
    it('keeps SCIM spotlight signals aligned with the canonical opportunity', () => {
      const scim = PM_DATA.opportunities.find((opportunity) => opportunity.id === 'o2')!
      const trending = PM_DATA.spotlight.trending.find((item) => item.oppId === 'o2')!
      const atRisk = PM_DATA.spotlight.atRisk.find((item) => item.oppId === 'o2')!

      expect(trending.stage).toBe(scim.stage)
      expect(trending.meta).toContain('164 conversations')
      expect(trending.meta).toContain(scim.revenue)
      expect(trending.trendPct).toBe(scim.volumePct)
      expect(trending.trendGood).toBe(scim.volumeGood)
      expect(atRisk.tag).toBe('gap')
      expect(atRisk.meta).toContain(`${scim.customers} customers`)
      expect(atRisk.meta).toContain(`Impact ${scim.impact}`)
      expect(atRisk.amount).toBe(scim.revenue)
      expect(PM_DATA.spotlight.asking.some((item) => item.oppId === 'o2')).toBe(false)
    })
  })
})
