...
describe('pm-detail-data', () => {
  ...
  it('detail records are well-formed', () => {
    for (const id of Object.keys(PM_OPPORTUNITY_DETAILS)) {
      ...
      if (d.opp.type === 'bug') expect(d.reproSteps?.length).toBeGreaterThan(0)
      if (d.opp.type === 'request') expect(d.signalEvidence?.length).toBeGreaterThan(0)
    }
  })
  ...
  it('keeps the SCIM request evidence coherent with provisioning', () => {
    const d = PM_OPPORTUNITY_DETAILS.o2
    expect(d.opp.impact).toBe(78)
    expect(d.volumeCount).toBe(164)
    expect(d.opp.type).toBe('request')
    expect(d.signalEvidence?.length).toBeGreaterThan(0)
    expect(d.conversations.map((c) => c.quote).join(' ')).toMatch(/provision|deprovision|roll out/i)
    expect(d.conversations.map((c) => c.quote).join(' ')).not.toMatch(/every 60 minutes|refresh silently/i)
