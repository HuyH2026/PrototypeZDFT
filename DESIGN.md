...
colors:
  ...
  accent-blue: "#406cc4"      # blue.700 (canonical Flora accent)
  ink: "#2f3130"              # grey.900
  ink-muted: "#5c6970"        # prototype's --fg-subtle; passes AA on white (5.9:1)
  nav-active: "#293239"       # app-specific dark chrome neutral
  app-backdrop: "#f7f7f7"     # grey.100 (flora-surface; glass cards require this backdrop)
  surface-border: "#e8eaec"   # card/table hairlines; #d8dcde remains as --border-default for controls
...
- **`ink-muted` (`grey.600` #5c6970)** — secondary/subtle text. Use for labels,
...
- **`app-backdrop` (#f7f7f7)** — the warm neutral page background behind cards.
- **`accent-blue` (#406cc4)** — the interactive/accent blue (links, primary
  actions), matching Flora's `blue.700`.
- **`surface-border` (#e8eaec)** — default card/table border. Note that
  `#d8dcde` remains available as `--border-default` for controls.

On top of the product tokens, a subset of the raw **Garden palette scales** is
exposed directly as Tailwind classes (`theme.css`), so screens use the exact
design-system value instead of transcribing raw hex: `grey-100` (#f7f7f7),
`grey-200` (#eae9e8), `grey-300` (#dcdcda), `grey-400` (#b7b7b3), `grey-500`
(#999b97), `grey-600` (#8b8e89), `grey-700` (#646864), `grey-800` (#404241),
`grey-900` (#2f3130), `grey-1200` (#0c0c0d), and `blue-700` (#406cc4 — the Flora
accent). Reach for these before an arbitrary `text-[#…]`/`bg-[#…]`; extend the
...
- **shadcn/ui kit** (`src/components/ui/`) is retained as a future toolkit; it
  uses shadcn base tokens (`--radius: 0.625rem`, etc.), which coexist with the
  Flora tokens. Prefer semantic token classes (`bg-nav-active`, `text-ink`,
  `border-surface-border`) over raw hex.

## Visual reference and primitive layer

The visual styling is ported from `zendesk/ft-unify` prototype `v1/index.html`
(the Flora flavor of Garden v10). The app implements Flora's semantic token
layer in `src/styles/theme.css` (foreground/background, warm-neutral palette,
warm translucent hovers, field tokens, table tokens, glass-card wash, status
scales, menu shadows, motion) and a set of presentational components in
`src/components/flora/`:

- `Card` — glass by default (frosted wash over transparent), `flat` prop opts out; **adopted** (39 files)
- `Button` — pill, four variants × three sizes; `primary` is dark neutral #2f3130; **not yet adopted**
- `Table`, `Thead`, `Tbody`, `Th`, `Td` — enforces the prototype's binding style rule; **adopted** (6 feature tables)
- `Field`, `TextArea` — hairline border + soft lift; **not yet adopted**
- `StatusTag` — pill + dot, eight states; **not yet adopted**
- `PageHeader` — sticky title over optional subtitle + actions; **not yet adopted**

## Known deltas (app vs canonical Garden/Flora)

The running app intentionally differs from canonical values in one area:

| Token | App value | Canonical | Note |
|-------|-----------|-----------|------|
| radii | shadcn `--radius: 0.625rem` | Flora `borderRadii` (md `8px`) | App runtime uses shadcn radii |

All color tokens now match the prototype (`accent-blue` #406cc4, `ink-muted`
#5c6970, `app-backdrop` #f7f7f7, `surface-border` #e8eaec).
